		...
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title">
					...
					<?php if(($list[0]["kqname"]) == "1"): echo (L("waiting_for_inspection")); endif; ?>
					<?php if(($list[0]["kqname"]) == "2"): echo (L("mainwarehouse")); endif; ?>
					<?php if(($list[0]["kqname"]) == "3"): echo (L("tobeprocessed")); endif; ?>
					<?php if(($list[0]["kqname"]) == "4"): echo (L("waste_warehouse")); endif; ?>
				</h2>
			</header>
							...
							<div class="modal-body" style="line-height: 34px;">
								<div class="row" style="margin-left: 0;margin-right: 0;">
									...
									<div class="input-group" style="padding-top: 20px;">
										...
										<input id="good" type="text" class="form-control" placeholder="" style="width: 80%;" value="<?php echo ($list[0]["namezh"]); ?>" readonly="readonly">
									</div>
									<div class="input-group" style="padding-top: 20px;">
									...
									<div class="input-group" style="padding-top: 20px;">
										<span class="input-group-addon"><?php echo (L("Number")); ?></span>
										<input id="num" type="text" class="form-control" placeholder="" style="width: 15%;" value="" readonly="readonly" name="xnum">
									</div>
									<div class="input-group" style="padding-top: 20px;">
										<span class="input-group-addon"><?php echo (L("current_warehouse")); ?></span>
										<input id="currentd" type="text" class="form-control" placeholder="" style="width: 70%;" value="<?php if(($_COOKIE['think_language']) == "zh-cn"): echo ($list[0]['zkname']); endif; if(($_COOKIE['think_language']) == "en-us"): echo ($list[0]['englishname']); endif; if(($list[0]["kqname"]) == "1"): echo (L("waiting_for_inspection")); endif; ?>
										<?php if(($list[0]["kqname"]) == "2"): echo (L("mainwarehouse")); endif; ?>
										<?php if(($list[0]["kqname"]) == "3"): echo (L("tobeprocessed")); endif; ?>
										<?php if(($list[0]["kqname"]) == "4"): echo (L("waste_warehouse")); endif; ?>" readonly="readonly">
										<input type="hidden" id="did" name="current">
									</div>
									<div class="input-group" style="padding-top: 20px;">

										<span class="input-group-addon"><?php echo (L("transfertothelibraryarea")); ?><span style="color: red;">*</span></span>
									...
									<div class="input-group spinner" data-trigger="spinner" style="padding-top: 20px;width: 80%;">
										<div class="input-group-btn">
											<div class="btn btn-default" style="width: 200px;background: #eee;"><?php echo (L("transfer_quantity")); ?><span style="color: red;">*</span></div>
			...
			<div class="panel-body">
				...
				<table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
					...
					<tbody>
					<input type="hidden" value="<?php echo ($list[0]['pid']); ?>" id="cpid">
					<input type="hidden" value="<?php echo ($list[0]['w_lid']); ?>" id="ckid">
						<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
								<input type="hidden" value="<?php echo ($k); ?>" class="bar">
								<td>
									<a href="<?php echo U('PurchaseOrder/completedDetails',array('id'=>$vo['o_id']));?>" target="_blank">PO<?php echo ($vo["order_number"]); ?></a>
								</td>
								<td class="nums"><?php echo ($vo["number"]); ?></td>
								<td>
											<button id="zq" onclick='areaturn("<?php echo ($vo["sfid"]); ?>","<?php echo ($vo["zk_id"]); ?>","<?php echo ($vo["pid"]); ?>","<?php echo ($vo["w_lid"]); ?>","<?php echo ($vo["number"]); ?>","<?php echo ($vo["o_id"]); ?>")' style="border: 0px;" type="button" class="btn btn-primary btn-xs">
										<?php echo (L("transit_zone")); ?>
									</button>
								</td>
		...
		$.ajax({
			...
			success: function(data) {
				if(data.status == 0) {
					...
					$('#node_message').html('<?php echo (L("successful_transfer")); ?>');
					node_message.style.display = 'block';
					var t = setTimeout("node_message.style.display='none';location.reload()", 2000);
				} else if(data == "no") {
					$('#node_message').html('<?php echo (L("transition_failed")); ?>');
					node_message.style.display = 'block';
					var t = setTimeout("node_message.style.display='none';", 2000);
				} else if(data == "so") {
					$('#node_message').html('<?php echo (L("zone_pool_area_cannot")); ?>');
					node_message.style.display = 'block';
					var t = setTimeout("node_message.style.display='none';", 2000);
				}
			}
		...
		$.ajax({
			...
			success: function(data) {
				...
				for(var i = 0; i < data.length; i++) {
					...
					if (data[i]['name']==1){
						var auu = '<?php echo (L("waiting_for_inspection")); ?>';
					}
					if (data[i]['name']==2){
						var auu = '<?php echo (L("mainwarehouse")); ?>';
					}
					if (data[i]['name']==3){
						var auu = '<?php echo (L("tobeprocessed")); ?>';
					}
					if (data[i]['name']==4){
						var auu = '<?php echo (L("waste_warehouse")); ?>';
					}
					var p = "<option value='" +
						data[i]['id'] +
						"'" +
