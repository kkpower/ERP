                ...
                <div>
                    <table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
                        ...
                        <?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
                                <td><a href="<?php echo U('Warehouse/countryExtend',array('id'=>$vo['id']));?>" target="_blank"><?php echo ($vo["countryzh"]); ?></a></td>
                                <td><a href="<?php echo U('Warehouse/countryExtend',array('id'=>$vo['id']));?>" target="_blank"><?php echo ($vo["country"]); ?></a></td>
                                <td>
                                   <?php echo ($vo["money"]); ?>
                                </td>
                                ...
                                <td>
                                    <button type="button" class="btn btn-primary btn-xs" title="<?php echo (L("Modify")); ?>" data-toggle="modal" onclick="ckedit('<?php echo ($vo["id"]); ?>','<?php echo ($vo["exchange_rate"]); ?>','<?php echo ($vo["money"]); ?>','<?php echo ($vo["currency"]); ?>')">
                ...
                <div class="modal-body">
                    <div class="input-group" style="float: left;padding: 0 20px 0;">
                        ...
                        <select  name="country_national" class="form-control" style="width: 50%;" id="country_national">
                            ...
                            <?php if(is_array($national)): $i = 0; $__LIST__ = $national;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$po): $mod = ($i % 2 );++$i;?><option value="<?php echo ($po["id"]); ?>"><?php echo ($po["namezh"]); ?>(<?php echo ($po["name"]); ?>)</option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select>
                    </div>
...
<script>
    ...
    function option_del(id) {
        ...
        if(r) {
            $.ajax({
                url: "<?php echo U('Warehouse/exchangerateDel');?>",
                type: "post", //选择传值方式
                data: {
                    id: id
                },
    ...
    //修改汇率弹框(赋值)
    function ckedit(id,rate,namezh,name){
        $('#hld').val(id);
        $('#rate1').val(rate);
        $('#monj').text(namezh+name);
    ...
    function doupdate(){
        $.ajax({
            url:"<?php echo U('Warehouse/exchangerateEdit');?>",
            type:"post",
            data:{
                id:$('#hld').val(),
                rate:$('#rate1').val()
    ...
    function doadd(){
        $.ajax({
            url:"<?php echo U('Warehouse/exchangerateAdd');?>",
            type:"post",
            data:{
                rate:$('#rate').val(),
