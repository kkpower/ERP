	...
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title">完整库存查询</h2>
			</header>
			<div class="panel-body">
				<form class="" action="<?php echo U('Warehouse/inLibrary','','');?>" method="get" style="margin-right:15px ;">
					<div class="col-md-3" style="padding-left: 0;">
						<div class="input-group">
							<span class="input-group-addon"><?php echo (L("warehouse")); ?></span>
							<input type="hidden" id="ck" value="<?php echo ($k_id); ?>">
							<select id="father" class="form-control" name="k_id">
								<option value="">--<?php echo (L("Pleaseselectawarehouse")); ?>--</option>
								<?php if(is_array($warehouse)): foreach($warehouse as $key=>$wa): ?><option value="<?php echo ($wa["id"]); ?>">
										<?php if(($_COOKIE['think_language']) == "zh-cn"): echo ($wa["name"]); endif; ?>
										<?php if(($_COOKIE['think_language']) == "en-us"): echo ($wa["englishname"]); endif; ?>
					...
					<div id="status_div" class="col-md-7" style="padding-left: 0;">
						<div class="input-group">
							...
							<input type="hidden" id="goodid" value="<?php echo ($p_id); ?>">
							<select name="p_id" id="product" class="form-control" data-live-search="true">
								<option value="">--<?php echo (L("Pleaseselectaproduct")); ?>--</option>
								<?php if(is_array($product)): $i = 0; $__LIST__ = $product;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pr): $mod = ($i % 2 );++$i;?><option value="<?php echo ($pr["id"]); ?>"><?php echo ($pr["bclassc_number"]); ?>.<?php echo ($pr["sclassc_number"]); ?>.<?php echo ($pr["snumber_number"]); ?>: <?php echo ($pr["namezh"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
							</select>
						</div>
					</div>
				...
				<table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
					<thead>
						<tr>
							...
							<th><?php echo (L("UnitPrice")); ?>
								<?php if($list[0]['spelling'] != ''): ?>(<?php echo ($list[0]['spelling']); ?>)<?php endif; ?>
							</th>
							<th style="cursor: pointer;"><?php echo (L("stock_quantity")); ?></th>
							<th><?php echo (L("frozen_quantity")); ?></th>
							<th><?php echo (L("Amount")); ?>
								<?php if($list[0]['spelling'] != ''): ?>(<?php echo ($list[0]['spelling']); ?>)<?php endif; ?>
							</th>
					...
					<tbody id="gbi">
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									<a href="<?php echo U('Product/homeProduct',array('pid'=>$vo['pid']));?>" target="_blank"><?php echo ($vo["namezh"]); ?></a>
								</td>
								<td>
									<?php echo ($vo["price"]); ?>
								</td>
								...
								<td><?php echo ($vo["freeze_number"]); ?></td>
								<td></td>
								<td>
									<a href="<?php echo U('Warehouse/areaReservoir',array('pid'=>$vo['pid'],'kid'=>$vo['zk_id']));?>" class="btn btn-primary btn-xs" style="color: #fff;" target="_blank"><?php echo (L("Details")); ?></a>
								</td>
				...
				<?php if($list): ?><div style="float: right;"><?php echo (L("Total")); ?>:<span style="font-size: 16px;color: red;" id="totalrmb"> <?php echo ($rmbtotal); ?></span><span id="rmb">RMB</span><span style="font-size: 16px;color: red;padding-left: 30px;" id="total"> <?php echo ($total); ?></span><?php echo ($list[0]['spelling']); ?></div><?php endif; ?>
...
<script>
	...
	if (currency=='RMB'){
		$('#totalrmb').hide();
		$('#rmb').hide();
		$('#total').css({'padding-left':'0px'});
	}else {
		$('#totalrmb').show();
		$('#rmb').show();
	}
	console.log($('#gbi').find('tr'));
	for(var i=0;i<$('#gbi').find('tr').length;i++){
		$('#gbi').find('tr')[i].children[2].innerText = parseInt(Number($('#gbi').find('tr')[i].children[2].innerText)*Number($('#country').val())*100+0.5)/100;
		var tby2 = $('#gbi').find('tr')[i].children[2].innerText;
		var tby3 = $('#gbi').find('tr')[i].children[3].innerText;
		$('#gbi').find('tr')[i].children[5].innerText=parseInt(Number(tby2)*Number(tby3)*100+0.5)/100;
	}
