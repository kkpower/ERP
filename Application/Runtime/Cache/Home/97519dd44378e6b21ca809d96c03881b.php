...
<!-- end: sidebar -->
    ...
    <div class="right-wrapper pull-right">
        ...
        <a class="sidebar-right-toggle" data-open="sidebar-right"><i class="fa fa-chevron-left"></i></a>
            ...
            <!-- start: page -->
            <input id="dell" type="hidden" value="<?php echo U('Employeelist/dell','','');?>">
...
<input type="hidden" id="transferuser" value="<?php echo U('Employeelist/transferuser','','');?>">
<input type="hidden" id="transferDou" value="<?php echo U('Employeelist/transferDou','','');?>">
		...
		<div class="modal-content">
			...
			<div class="modal-body" style="line-height: 34px;">
				<div class="row">
					...
					<div class="input-group col-md-6" style="float: left;padding: 0 15px;margin-bottom: 12px;">
						<span class="input-group-addon"><?php echo (L("Transfercompany")); ?></span>
						<select id="leader2" name="company_id" onchange="searchAjax2()" class="form-control">
							...
							<?php if(is_array($organization)): foreach($organization as $key=>$or): ?><option value="<?php echo ($or["id"]); ?>"><?php echo ($or["namezh"]); ?></option><?php endforeach; endif; ?>

						</select>
					</div>
					<div class="input-group col-md-6" style="float: left;padding: 0 15px;margin-bottom: 12px;">
						<span class="input-group-addon"><?php echo (L("Transferdepartment")); ?></span>
					...
					<div class="input-group col-md-6" style="float: left;padding: 0 15px;">
						<span class="input-group-addon"><?php echo (L("Transferposition")); ?></span>
	...
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title"><?php echo (L("Personneldirectory")); ?></h2>
			</header>
			<div class="panel-body">
				<input id="searchAjax" type="hidden" value="<?php echo U('Employeelist/getAjaxOrganization','','');?>">
				<form class="" action="<?php echo U('Employeelist/index','','');?>" method="get" style="display: inline-block;float: left;margin-right:15px;">
					<div class="input-group" style="width: 500px;">
						<input name="search_term" type="text" placeholder="<?php echo (L("Pleaseenter")); ?>" value="" class="form-control">
						<span class="input-group-btn">
		                    <button type="submit" class="btn btn-success" style="white-space: nowrap;"><?php echo (L("Search")); ?></button>
		                </span>
					</div>
				</form>
				<button class="btn btn-primary" onclick="onm()" style="float: left;"><?php echo (L("Addemployee")); ?></button>
				<table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
					...
					<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									<button onclick="transferuser('<?php echo ($vo["id"]); ?>')" class="btn btn-xs btn-primary"><?php echo (L("Transfer")); ?></button>
									<button onclick="option('<?php echo ($vo["id"]); ?>')" class="btn btn-xs btn-primary"><?php echo (L("Details")); ?></button>
									<button onclick="option_del('<?php echo ($vo["id"]); ?>')" class="btn btn-xs btn-danger"><?php echo (L("Delete")); ?></button>
								</td>
...
<!--模态框-->
<form id="form" action="<?php echo U('Employeelist/add','','');?>" method="post">
	<div id="node_modal" class="modal fade  bs-example-modal-lg" tabindex="-1" role="dialog">
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
					...
					<div class="row" style="margin-left: 0;margin-right: 0;">
						...
						<div class="input-group" style="padding-top: 20px;">
							...
							<div style="border: 1px solid #ccc;width: 30%;border-bottom-right-radius: 4px;border-top-right-radius: 4px;">
								...
								<input type="radio" id="sex_man" style="width: 16px;height: 16px;vertical-align: top;margin-top: 8px;margin-left: 20px;" name="sex" value="1" checked="checked"><?php echo (L("Man")); ?>
							</label>
								<label for="sex_woman" style="margin-bottom: 0;">
								<input type="radio" id="sex_woman" style="width: 16px;height: 16px;vertical-align: top;margin-top: 8px;margin-left: 5px;" name="sex" value="2"><?php echo (L("Woman")); ?>
							</label>
							</div>
						</div>
						<div class="col-md-12">
				...
				<div class="modal-footer">
					<button type="submit" class="btn btn-primary" onclick="aa()"><?php echo (L("Save")); ?></button>
					<button type="button" class="btn btn-default" data-dismiss="modal"><?php echo (L("Cancel")); ?></button>
				</div>
			</div>
			<!-- /.modal-content -->
...
<script>
	...
	function positionAjax() //第三级的下拉
	{
		...
		$.ajax({
			...
			success: function(data) {
				console.log(data);
				var p = "<option id='ozhi' value=''><?php echo (L("Pleaseselectposition")); ?></option>"
				$("#zhiwu").html(p);
				//将获取到的数据赋值
				for(var i = 0; i < data.length; i++) {
					//拼接option标签的 name value 等属性
