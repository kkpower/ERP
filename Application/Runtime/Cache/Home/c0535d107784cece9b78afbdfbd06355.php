	...
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title"><?php echo (L("order_history_list")); ?><span style="color: red;"> <?php echo ($id); ?> </span><?php echo (L("batch")); ?></h2>
				<div style="float: left;">
					<p style="font-size: 14px;margin-bottom: 0;color: #666;"><?php echo (L("this_time_import")); ?><span style="font-weight: bold;color: red;"> <?php echo ($count); ?> </span><?php echo (L("orders")); ?></p>
					<p style="font-size: 14px;margin-bottom: 0;color: #666;"><?php echo (L("order_status")); ?>:<span style="font-weight: bold;">
                     <?php if(is_array($problem)): foreach($problem as $key=>$vo): echo ($key); ?>:<?php echo ($vo); endforeach; endif; ?>
                    </span></p>
					<!--<p style="font-size: 14px;margin-bottom: 0;color: #666;">合并订单:<span style="font-weight: bold;"><?php echo ($merge); ?></span></p>-->
				</div>
				<div style="margin-left: 20px;float: left;">
					<div>
						<?php if(is_array($countryGP)): foreach($countryGP as $key=>$vo): ?><p  style="font-size: 14px;margin-bottom: 0;color: #666;">
								<?php echo (L("receipt_area")); ?>:<span style="font-weight: bold;"><?php echo ($key); ?>:<?php echo ($vo); ?></span>
			...
			<div class="panel-body">
				<form id="country_form" action="<?php echo U('Order/importlist','','');?>" method="get" style="float: left;min-width: 240px;">
					<input type="hidden" value="<?php echo ($acid); ?>" name="acid">
					<input type="hidden" value="<?php echo ($id); ?>" name="id">
					<div class="input-group" style="float: left;width: 320px;margin-right: 10px;">
						<div class="input-group">
							<span class="input-group-addon"><?php echo (L("account")); ?></span>
							<input readonly="readonly" class="form-control" type="text" value="<?php echo ($platformaccount); ?>:<?php echo ($platform); ?>">
						</div>
					</div>
				...
				<table id="table1" class="table table-hover table-bordered table-striped" style="border-collapse: collapse;">
					...
					<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td id="<?php echo ($key); ?>_2"><?php echo ($vo["external_sn"]); ?></td>
...
<script>
	...
	function search_order() {
		window.open("<?php echo U('Order/details');?>?o_number=" + $('#search_order').val());
	}
