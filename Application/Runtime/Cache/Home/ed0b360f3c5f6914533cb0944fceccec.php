		...
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title">
					<?php if(($_COOKIE['think_language']) == "zh-cn"): echo ($name); endif; ?>
					<?php if(($_COOKIE['think_language']) == "en-us"): echo ($uname); endif; ?>
					<?php echo (L("warehouse")); ?> -- <?php echo (L("historical_order")); ?></h2>
			</header>
			<div class="panel-body">
				<div id="node_message" style="position: fixed;top: 50%;left: 50%;display: none;background: #888;min-width:300px;margin-left: -150px;margin-top:-15px;border-radius: 8px;text-align: center;font-size: 16px;padding: 12px 20px;color: #fff;z-index: 1200;"></div>
				<form class="" action="<?php echo U('PurchaseOrder/historicalOrder');?>" method="get" style="display: inline-block;float: left;margin-right:15px ;">
					<!--<div id="status_div" class="col-md-5">-->
						<!--<div class="input-group">-->
							...
							<!--<select name="supplier_id" id="supplier" class="form-control" data-live-search="true">-->
								...
								<!--<?php if(is_array($supplier)): $i = 0; $__LIST__ = $supplier;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$pr): $mod = ($i % 2 );++$i;?>-->
							...
							<!--&lt;!&ndash;<?php if(is_array($supplier)): foreach($supplier as $key=>$wa): ?>&ndash;&gt;-->
							<!--&lt;!&ndash;<option value="<?php echo ($wa["id"]); ?>"><?php echo ($wa["name"]); ?></option>&ndash;&gt;-->
					...
					<div class="input-group" style="width: 400px;float: left;">
						...
						<input type="hidden" value="<?php echo ($oid); ?>" id="purchsearch">
						...
						<!--<select class="form-control" name="status" id="orderall">-->
							<!--<option value="">--全部--</option>-->
							<!--<?php if(is_array($status_ex)): foreach($status_ex as $key=>$or): ?>-->
								<!--<option value="<?php echo ($or["id"]); ?>"><?php echo ($or["namezh"]); ?></option>-->
				...
				<table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
					...
					<tbody id="tbody-resultd">
						<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
								...
								<td>PO<?php echo ($vo["order_id"]); ?></td>
								<td>
									<a href="<?php echo U('PurchaseOrder/completedDetails',array('id'=>$vo['id']));?>" target="_blank" style="border: 0px;color: #fff;" class="btn btn-primary btn-xs"><?php echo (L("Details")); ?></a>
								</td>
...
<script>
	...
	function mpdf(id) {
		...
		var url = "<?php echo U('PurchaseOrder/purchase_successOrderPdf');?>";
		form.attr({
			"action": url
		});
