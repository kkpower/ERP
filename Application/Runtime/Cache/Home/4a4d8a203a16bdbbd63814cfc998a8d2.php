	...
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title">  <?php if(($_COOKIE['think_language']) == "zh-cn"): echo ($physical); endif; if(($_COOKIE['think_language']) == "en-us"): echo ($physicalus); endif; echo (L("reservoir_management")); ?></h2>
			</header>
			<div class="panel-body">
				...
				<!--<div style="padding-top: 20px;">-->
					<table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
						...
						<?php if(is_array($warehouse)): foreach($warehouse as $k=>$vo): ?><tr>
								<input type="hidden" value="<?php echo ($k); ?>" name="type1">
								<td>
									<?php if(($vo["w_type"]) == "1"): echo (L("waiting_for_inspection")); endif; ?>
									<?php if(($vo["w_type"]) == "2"): echo (L("mainwarehouse")); endif; ?>
									<?php if(($vo["w_type"]) == "3"): echo (L("tobeprocessed")); endif; ?>
									<?php if(($vo["w_type"]) == "4"): echo (L("waste_warehouse")); endif; ?>
									<!--<?php if(($vo["type"]) == ""+vo.type+""): echo ($vo["type"]); endif; ?>-->
								</td>
								<td>
								...
								<td>
									<?php if($vo["w_type"] != '1' ): if(($vo["status"]) == "1"): ?><button type="button" class="btn btn-danger btn-xs"  onclick="ckedit('<?php echo ($vo["id"]); ?>','2')"><?php echo (L("locking")); ?></button><?php endif; ?>
										<?php if(($vo["status"]) == "2"): ?><button type="button" class="btn btn-success btn-xs"  onclick="ckedit('<?php echo ($vo["id"]); ?>','1')"><?php echo (L("normal")); ?></button><?php endif; endif; ?>
									<?php if($vo["w_type"] == '1' ): ?><button type="button" class="btn btn-danger btn-xs" disabled="disabled"><?php echo (L("locking")); ?></button><?php endif; ?>
								</td>
			...
			<form class="form-horizontal" onsubmit="return doadd(this)">
				<div class="modal-body">
					<div class="input-group" style="padding: 0 20px 0;">
						<input type="hidden" value="<?php echo ($kid); ?>" name="ku_id">
...
<script>
    ...
    function ckedit(id,status){
    	...
    	if(r == true){
    		$.ajax({
            url:"<?php echo U('Warehouse/warehouseUpdate');?>",
            type:"post",
            data:{
            	id: id,
            	status: status
            ...
            success:function(data){
                ...
                if (data.status==0){
					...
					$('#node_message').html('<?php echo (L("locked_successfully")); ?>');
					node_message.style.display = 'block';
					var t = setTimeout("node_message.style.display='none';location.reload()", 1500);
				}else if (data=='no'){
