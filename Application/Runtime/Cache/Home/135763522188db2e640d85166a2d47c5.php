	...
	<div class="col-md-12">
		<section class="panel">
			...
			<div class="panel-body">
				<form class="" action="<?php echo U('Supplier/index');?>" method="get" style="display: inline-block;float: left;margin-right:15px ;">
					<div class="input-group" style="width: 500px;">
						<input name="search" type="text" value="<?php echo ($suppliername); ?>" class="form-control" placeholder="<?php echo (L("enter_name_vendor_youwant")); ?>">
						<span class="input-group-btn">
		                    <button type="submit" class="btn btn-success" style="white-space: nowrap;"><?php echo (L("Search")); ?></button>
		                </span>
					</div>
				</form>
				<button class="btn btn-primary" onclick="add()" style="float: left;"><?php echo (L("add_supplier")); ?></button>
				<br style="clear: both;" />
				<table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
					<thead>
						<tr>
							<th style="cursor: pointer;"><?php echo (L("supplier_name")); ?><span class="caret"></span></th>
							<th><?php echo (L("supplier_type")); ?></th>
					...
					<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<input type="hidden" value="<?php echo ($vo["snumber"]); ?>" name="pao">
								<td>
									<?php echo ($vo["name"]); ?>
								</td>
								<td>
								...
								<td>
									<?php if(($_COOKIE['think_language']) == "zh-cn"): echo ($vo["namech"]); endif; ?>
									<?php if(($_COOKIE['think_language']) == "en-us"): echo ($vo["nameuk"]); endif; ?>
								</td>
								<td>
									<a href="<?php echo U('Supplier/proDuct',array('id'=>$vo['id']));?>" class="btn btn-primary btn-xs" style="color: #fff;" target="_blank"><?php echo (L("product")); ?></a>
									<a href="<?php echo U('Supplier/detail',array('id'=>$vo['id']));?>" class="btn btn-primary btn-xs" style="color: #fff;" target="_blank"><?php echo (L("view")); ?></a>
									<!--<button class="btn btn-danger btn-xs"  onclick='gysdel(this,"<?php echo ($vo["id"]); ?>")'><?php echo (L("Delete")); ?></button>-->
									<?php if(($vo["status"]) == "1"): ?><button type="button" class="btn btn-danger btn-xs" onclick="gysdel('<?php echo ($vo["id"]); ?>',1)"><?php echo (L("Disable")); ?></button><?php endif; ?>
									<?php if(($vo["status"]) == "2"): ?><button type="button" class="btn btn-success btn-xs" onclick="gysdel('<?php echo ($vo["id"]); ?>',2)"><?php echo (L("Enable")); ?></button><?php endif; ?>
								</td>
					...
					<div class="row" style="margin-left: 0;margin-right: 0;">
						...
						<div class="input-group" style="padding-top: 12px;">
							...
							<select type="text" name="type" class="form-control">
								<option value=""><?php echo (L("pleaseselectsuppliertype")); ?></option>
						...
						<div class="input-group" style="padding-top: 12px;">
							<span class="input-group-addon"><?php echo (L("Vendorname")); ?><span style="color: red;">*</span></span>
						...
						<div class="input-group" style="padding-top: 12px;">
							...
							<select type="text" name="payment" class="form-control" placeholder="Last Name">
								<option value=""><?php echo (L("pleaseselectthepaymentmethod")); ?></option>
						...
						<div class="input-group" style="padding-top: 12px;">
							...
							<select type="text" name="state" class="form-control">
								...
								<option value="2"><?php echo (L("Disable")); ?></option>
							</select>
						</div>
					</div>
				</div>
...
<script>
	...
	function doadd(form) {
		$.ajax({
			url: "<?php echo U('Supplier/supplierAdd');?>",
			type: "post",
			data: $(form).serialize(),
			dataType: "json",
			async: true,
	...
	function gysdel(id, type) {
		...
		if(r) {
			$.ajax({
				url: "<?php echo U('Supplier/gysdelete');?>",
				type: 'post',
				dataType: "json",
				data: {
					id: id,
