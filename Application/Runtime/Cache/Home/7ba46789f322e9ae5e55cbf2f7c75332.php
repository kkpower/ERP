...
<!--获取收货信息-->
<input type="hidden" id="ajaxReceipt" value="<?php echo U('PurchaseOrder/ajaxReceipt','','');?>">
			...
			<div class="panel-body">
					...
					<!--<div class="col-md-3">-->
						<!--<div class="input-group">-->
							...
							<!--<select name="zk_id" id="father" class="form-control">-->
								...
								<!--<?php if(is_array($warehouse)): foreach($warehouse as $key=>$wa): ?>-->
									<!--<option value="<?php echo ($wa["id"]); ?>"><?php echo ($wa["name"]); ?></option>-->
				...
				<div id="box_info" style="width: 100%;display: none;">
					...
					<div style="padding-top: 10px;">
						<div class="col-md-6">
							<div class="details">
								<span><?php echo (L("Purchaseordernumber")); ?>：</span>
								<div id="ordernum"></div>
							</div>
						</div>
						<div class="col-md-6">
							<div class="details">
								<span><?php echo (L("TrackingNumber")); ?>：</span>
								<div id="track"></div>
							</div>
						</div>
						<div class="col-md-6">
							<div class="details">
								<span><?php echo (L("from_supplier")); ?>：</span>
								<div id="supplier2"></div>
							</div>
						</div>
						<div class="col-md-6">
							<div class="details">
								<span><?php echo (L("Receivingwarehouse")); ?>：</span>
								<div id="ware"></div>
							</div>
						</div>
						<div class="col-md-6">
							<div class="details">
								<span><?php echo (L("Transporters")); ?>：</span>
								<div id="yss"></div>
							</div>
						</div>
						<div class="col-md-6">
							<div class="details">
								<span><?php echo (L("Modeoftransport")); ?>：</span>
								<div id="tran"></div>
							</div>
						</div>
						<div class="col-md-6">
							<div class="details">
								<span><?php echo (L("delivery_time")); ?>：</span>
								<div id="futime"></div>
							</div>
						</div>
						<br style="clear: both;" />
						...
						<button class="btn btn-primary" style="margin-right: 15px;" onclick="confirmReceipt()"><?php echo (L("receipt")); ?></button>
						<button class="btn btn-default" id="close"><?php echo (L("Close")); ?></button>
					</div>
				</div>
			</div>
...
<script>
			...
			$.ajax({
				...
				success: function(data) {
					...
					if(data.info) {
						...
						$('#ordernum').html("<a href=\"<?php echo U('PurchaseOrder/completedDetails','','');?>/id/"+data.two.id+"\" target=\"_blank\">"+'PO'+data.two.order_number+"</a>");
						$('#supplier2').html("<a href=\"<?php echo U('Supplier/detail','','');?>/id/"+data.two.supplier_id+"\" target=\"_blank\">"+data.two.supplier_name+"</a>");
						$('#ware').html("<a href=\"<?php echo U('Warehouse/detail','','');?>/id/"+data.two.ku_id+"\" target=\"_blank\">"+data.two.ck_name+"</a>");
						...
						for(var i = 0; i < data.one.length; i++) {
							list += '<tr>' +
									'<td>' + "<a href=\"<?php echo U('Product/homeProduct','','');?>/pid/"+data['one'][i]['pid']+"\" target=\"_blank\">" + data['one'][i].bclassc_number +'.'+ data['one'][i].sclassc_number +'.'+ data['one'][i].snumber_number + "</a>"+ '</td>' +
									'<td>' + "<a href=\"<?php echo U('Product/homeProduct','','');?>/pid/"+data['one'][i]['pid']+"\" target=\"_blank\">" + data['one'][i].namezh +"</a>"+ '</td>' +
	...
	function confirmReceipt(){
		...
		if (r){
			...
			$.ajax({
				url: "<?php echo U('PurchaseOrder/collectDo');?>",
				type: 'post',
				dataType: "json",
				async: true,
				data: {
