	...
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title"><?php echo (L("order_history")); ?></h2>
			</header>
			<div class="panel-body">
				<div id="node_message" style="position: fixed;top: 50%;left: 50%;display: none;background: #888;min-width:300px;margin-left: -150px;margin-top:-15px;border-radius: 8px;text-align: center;font-size: 16px;padding: 12px 20px;color: #fff;z-index: 1200;"></div>
				<form id="form" action="<?php echo U('Order/importHistory');?>" method="GET" enctype="multipart/form-data">
					<div class="input-group" style="float: left;width: 240px;margin-right: 10px;">
						<span class="input-group-addon"><?php echo (L("Platform")); ?></span>
					...
					<div class="input-group" style="float: left;width: 240px;">
						<span class="input-group-addon"><?php echo (L("select_account")); ?></span>
				...
				<table id="table1" class="table table-hover table-bordered" style="border-collapse: collapse;">
					<thead>
						<tr>
							...
							<td><?php echo (L("Id")); ?></td>
							<!--<td>导入数量</td>-->
							<td><?php echo (L("batch")); ?></td>
							<!--<td>文件名</td>-->
							<td><?php echo (L("filename")); ?></td>
                            <!--<td>导入时间</td>-->
							<td><?php echo (L("import_time")); ?></td>
                            <!--<td>操作</td>-->
					...
					<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td id="<?php echo ($key); ?>_2"><?php echo ($vo["ctime"]); ?></td>
								<td id="<?php echo ($key); ?>_11">
									<a style="color: #fff;" title="<?php echo (L("view_details")); ?>" type="button" target="_blank" href="<?php echo U('Order/importlist',array('id'=>$vo['id'],'acid'=>$vo['ac_id']));?>" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-search"></span></a>
									<!--<a style="color: #fff;" type="button" target="_blank" href="<?php echo U('Order/details',array('id'=>$vo['id']));?>" class="btn btn-info btn-xs">订单详情</a>-->
									<?php if(($vo["status_id"] < 3) OR ($vo["status_id"] == 9)): ?><button onclick='historydel("<?php echo ($vo["id"]); ?>")' title="<?php echo (L("Delete")); ?>" style="background: #EE0000; border: 0px;" type="button"data-toggle="modal" class="btn btn-primary btn-xs" onclick="adclass(2,'<?php echo ($vo["id"]); ?>')" >
...
<script type="text/javascript">
	...
	function get_two() {
		$.ajax({
			...
			success: function(data) {
				console.log(data)
				//将获取到的数据赋值
				var p = "<option value=''><?php echo (L("pselect_account")); ?></option>"
	...
	function historydel(id){
		var r = confirm("<?php echo (L("delete_historical_order")); ?>");
		if (r){
			$.ajax({
				...
				success:function(data){
					...
					if (data==null){
						...
						$('#node_message').html('<?php echo (L("order_not_deleted")); ?>');
						node_message.style.display = 'block';
						var t = setTimeout("node_message.style.display='none';", 2000);
						...
						$('#node_message').html("<?php echo (L("Delete")); ?>"+data.branch+"<?php echo (L("orders")); ?>"+" , "+"<?php echo (L("residue")); ?>"+data.surplus+"<?php echo (L("orders")); ?>");
						node_message.style.display = 'block';
