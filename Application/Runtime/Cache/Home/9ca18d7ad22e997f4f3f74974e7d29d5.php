	...
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title"><?php echo (L("order_shipped")); ?></h2>
			</header>
			<div class="panel-body">
				<div id="node_message" style="position: fixed;top: 50%;left: 50%;display: none;background: #888;min-width:300px;margin-left: -150px;margin-top:-15px;border-radius: 8px;text-align: center;font-size: 16px;padding: 12px 20px;color: #fff;z-index: 1200;"></div>
				<form class="" action="<?php echo U('PurchaseOrder/completeOrder');?>" method="get" style="display: inline-block;float: left;margin-right:15px ;">
				...
				<table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
					...
					<tbody id="tbody-resultd">
						<?php if(is_array($purchaselist)): $k = 0; $__LIST__ = $purchaselist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
								...
								<td>
									<?php echo ($vo["ysfxname"]); ?>
								</td>
								<td class="delive"><?php echo ($vo["delivery_date"]); ?></td>
								<td><?php echo ($vo["lastnamezh"]); echo ($vo["namezh"]); ?></td>
								<td><?php if(($vo["status"]) == "2"): echo (L("receivedall")); else: echo (L("not_all_received")); endif; ?></td>
								<!--<td><?php echo ($vo["creationtime"]); ?></td>-->
								<td>
										<button onclick='mpdf("<?php echo ($vo["id"]); ?>")' style="border: 0px;" type="button" class="btn btn-primary btn-xs">
										pdf
									</button>
