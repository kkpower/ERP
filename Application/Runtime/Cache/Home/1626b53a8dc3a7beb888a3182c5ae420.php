	...
	<div class="col-md-6">
		<section class="panel">
			...
			<div class="panel-body">
				<form id="form1" action="<?php echo U('Config/transportList');?>" method="get">
					<div class="input-group" style="width: 320px;float: left;">
						...
						<select id="country" name="country" class="form-control" onchange="$('#form1').submit()">
							...
							<?php if(is_array($country)): foreach($country as $key=>$vo): ?><option data-id="<?php echo ($vo["default_transport_id"]); ?>" id="country_<?php echo ($vo["cid"]); ?>" value="<?php echo ($vo["cid"]); ?>"><?php echo ($vo["countryus"]); ?></option><?php endforeach; endif; ?>
						</select>
					</div>
				</form>
				...
				<table id="table1" class="table mb-none table-hover table-striped" style="text-align: center;border-collapse: collapse;margin-top: 15px;">
					...
					<tbody id="transport">
						<?php if(is_array($transporters)): $i = 0; $__LIST__ = $transporters;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td style="line-height: 26px;"><input type="checkbox" id="<?php echo ($vo["tid"]); ?>_ck1" name="checkboxed1" class="row_check row_check1" style="width: 20px;height: 20px;"></td>
								<td><?php echo ($key+1); ?></td>
								<td><?php echo ($vo["transporters"]); ?></td>
								<td><input type="radio" data-id="<?php echo ($vo["default_mode_id"]); ?>" class="tp_radio" name="tp_radio" id="tp_<?php echo ($vo["tid"]); ?>" /></td>
								<td>
									<button type="button" onclick="transport_mode(this,'<?php echo ($vo["tid"]); ?>')" class="btn btn-info btn-xs">
                                        	查看
                                    </button>
									<button type="button" onclick="editTransporters1(this,'<?php echo ($vo["tid"]); ?>')" class="btn btn-primary btn-xs">
                                        	修改
                                    </button>
								</td>
...
<div class="modal fade" id="addModeModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog" style="width: 500px;">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" onclick="addMode()"><?php echo (L("Save")); ?></button>
				<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
			</div>
		</div>
		<!-- /.modal-content -->
...
<div class="modal fade" id="editModeModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog" style="width: 500px;">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" onclick="editMode()"><?php echo (L("Save")); ?></button>
				<button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
			</div>
		</div>
		<!-- /.modal-content -->
...
<script>
	...
	if(country != "") {
		...
		$.ajax({
			url: "<?php echo U('Config/getAjaxTransport');?>", //通过JQ获取URL获得路径
			data: {
				cid: country
			}, //通过页面元素的ID来获取设备ID
			type: "post", //选择传值方式(
	...
	function transport_mode(that, id) {
		...
		$.ajax({
			url: "<?php echo U('Config/getAjaxMode');?>", //通过JQ获取URL获得路径
			data: {
				tid: id
			}, //通过页面元素的ID来获取设备ID
			type: "post", //选择传值方式(
			...
			success: function(data) {
				...
				$.ajax({
					url: "<?php echo U('Config/getAjaxStatus');?>", //通过JQ获取URL获得路径
					data: {
						id: id
					}, //通过页面元素的ID来获取设备ID
					type: "post", //选择传值方式(
	...
	function addTransporters() {
		...
		$.ajax({
			url: "<?php echo U('Config/addTransporters');?>", //通过JQ获取URL获得路径
			data: {
				cid: $('#area').val(),
	...
	function addMode() {
		...
		$.ajax({
			url: "<?php echo U('Config/addMode');?>", //通过JQ获取URL获得路径
			data: {
	...
	function delTransporters(id) {
		...
		if(r == true) {
			$.ajax({
				url: "<?php echo U('Config/delTransporters');?>", //通过JQ获取URL获得路径
				data: {
					tid: id,
					cid: $('#area').val()
				}, //通过页面元素的ID来获取设备ID
	...
	function delMode(id) {
		...
		if(r == true) {
			$.ajax({
				url: "<?php echo U('Config/delMode');?>", //通过JQ获取URL获得路径
				data: {
					id: id
				}, //通过页面元素的ID来获取设备ID
				type: "post", //选择传值方式(
	...
	function editTransporters() {
		$.ajax({
			url: "<?php echo U('Config/editTransporters');?>", //通过JQ获取URL获得路径
			data: {
				cid: country,
	...
	function updateTransporters() {
		...
		console.log(arr2);
		$.ajax({
			url: "<?php echo U('Config/updateTransporters');?>", //通过JQ获取URL获得路径
			data: {
				cid: $('#area').val(),
				tid: arr3,
				status: arr1,
