	...
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title"><?php echo (L("pending_receipt")); ?></h2>
			</header>
			<div class="panel-body">
				<form class="form-horizontal" action="<?php echo U('Warehouse/warehouSing','','');?>" method="get">
					<div class="row">
						<div class="col-md-3">
							<div class="input-group">
								<span class="input-group-addon"><?php echo (L("ordernumber")); ?></span>
								<input type="hidden" id="orderon" value="<?php echo ($orderon); ?>">
								<input id="odid" name="orderon" type="text" value="" class="form-control" placeholder="<?php echo (L("Pleaseordernumber")); ?>">
							</div>
						</div>
						<div class="col-md-3">
							<div class="input-group">
								<span class="input-group-addon"><?php echo (L("supplier")); ?></span>
								...
								<select name="supplier" id="supper" class="form-control">
									...
									<?php if(is_array($supplier)): foreach($supplier as $key=>$su): ?><option value="<?php echo ($su["id"]); ?>"><?php echo ($su["name"]); ?></option><?php endforeach; endif; ?>
								</select>
							</div>
						</div>
						<div class="col-md-3">
							<div class="input-group">
								<span class="input-group-addon"><?php echo (L("sent_to_the_warehouse")); ?></span>
								<input type="hidden" id="mbck" value="<?php echo ($zkid); ?>">
								<select name="zk_id" id="father" class="form-control">
									<option value=""><?php echo (L("All")); ?></option>
									<?php if(is_array($warehouse)): foreach($warehouse as $key=>$wa): ?><option value="<?php echo ($wa["id"]); ?>"><?php echo ($wa["name"]); ?></option><?php endforeach; endif; ?>
								</select>
							</div>
						</div>
						<!--<div class="col-md-3">-->
							<!--<div class="input-group">-->
								...
								<!--<input type="hidden" id="ysfs" value="<?php echo ($type); ?>">-->
								<!--<select class="form-control" id="type" name="type">-->
									<!--<option value=""><?php echo (L("All")); ?></option>-->
									<!--<?php if(is_array($transport)): foreach($transport as $key=>$ys): ?>-->
										<!--<option value="<?php echo ($ys["id"]); ?>"><?php echo ($ys["mode"]); ?></option>-->
									<!--<?php endforeach; endif; ?>-->
								<!--</select>-->
							<!--</div>-->
						<!--</div>-->
						  <span class="input-group-btn">
								<button type="submit" class="btn btn-success"><?php echo (L("Search")); ?></button>
                           </span>
					</div>
				</form>
				...
				<div>
					<table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
						<thead>
							<tr>
								...
								<th><?php echo (L("kind")); ?></th>
								<th><?php echo (L("from_supplier")); ?></th>
						...
						<tbody>
							...
							<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr>
									<td><a href="<?php echo U('PurchaseOrder/completedDetails',array('id'=>$vo['oid']));?>" target="_blank">PO<?php echo ($vo["order_number"]); ?></a></td>
									<td><?php echo ($vo["number"]); ?></td>
									<td><a href="<?php echo U('Supplier/detail',array('id'=>$vo['supplier_id']));?>" target="_blank"><?php echo ($vo["supname"]); ?></a></td>
									<td>
										<?php echo ($vo["name"]); ?>
									</td>
									<td><?php echo ($vo["yssname"]); ?></td>
									<td>
										<?php echo ($vo["namezh"]); ?>
									</td>
									<td><?php echo ($vo["examine_time"]); ?></td>
									<td>
										<button onclick='collectGood("<?php echo ($vo["zku_id"]); ?>","<?php echo ($vo["ku_id"]); ?>","<?php echo ($vo["logistics_status"]); ?>","<?php echo ($vo["order_number"]); ?>","<?php echo ($vo["supname"]); ?>","<?php echo ($vo["name"]); ?>","<?php echo ($vo["oid"]); ?>","<?php echo ($vo["namezh"]); ?>","<?php echo ($vo["examine_time"]); ?>","<?php echo ($vo["yssname"]); ?>","<?php echo ($vo["tracking_number"]); ?>","<?php echo ($vo["delivery_date"]); ?>")' style="border: 0px;" type="button" class="btn btn-primary btn-xs">
                                       <?php echo (L("receipt")); ?>
                                    </button>
					...
					<!--<ul class="pager" style="width: 100%;">-->
						<!--&lt;!&ndash; 分页显示 &ndash;&gt;<?php echo ($page); ?></ul>-->
				</div>
			</div>
		...
		<div class="modal-dialog modal-lg" role="document">
			<div class="modal-content">
				...
				<div class="modal-body" style="line-height: 34px;">
					<div class="row" style="margin-left: 0;margin-right: 0;">
						<table id="table3" class="table table-hover table-striped table-bordered" style="margin-bottom: 0;border-collapse: collapse;">
							<thead>
							<tr>
								<th><?php echo (L("Purchaseordernumber")); ?></th>
								<th><?php echo (L("TrackingNumber")); ?></th>
								<th><?php echo (L("from_supplier")); ?></th>
								<th><?php echo (L("Receivingwarehouse")); ?></th>
								<th><?php echo (L("Transporters")); ?></th>
								<th><?php echo (L("Modeoftransport")); ?></th>
								<th><?php echo (L("estimated_delivery_time")); ?></th>
								<th><?php echo (L("delivery_time")); ?></th>
							</tr>
							<tr>
								<td id="ordernum"></td>
						...
						<table id="table2" class="table table-hover table-striped table-bordered" style="margin-bottom: 0;border-collapse: collapse;margin-top: 20px;">
							<thead>
								<tr>
									<th><input type="checkbox" class="check-all" style="vertical-align: middle;margin: 0;"><?php echo (L("select_all")); ?></th>
									<th><?php echo (L("product_number")); ?></th>
									<th><?php echo (L("Productname")); ?></th>
									<th id="priced"></th>
									<th><?php echo (L("expected_quantity")); ?></th>
									<th><?php echo (L("actual_quantity")); ?></th>
									<th id="current"></th>
								</tr>
							</thead>
				...
				<div class="modal-footer">
					...
					<button class="btn btn-primary" id="add_pr" type="submit" onclick="dogoods()"><?php echo (L("confirm")); ?></button>
...
<script>
	...
	function collectGood(zku_id,ku_id,logistics_status,odnumber,namezhs,name,oid,namezh,futime,yssname,tracking_number,delivery_date) {
		...
		tbody.innerHTML='';
		$.ajax({
			url: "<?php echo U('Warehouse/collectGoods');?>",
			type: 'post',
			data: {
				oid: oid
			},
			...
			success: function(data) {
				$('#priced').text("<?php echo (L("UnitPrice")); ?>"+'('+data[0]['currency']+')');
				$('#current').text("<?php echo (L("Amount")); ?>"+'('+data[0]['currency']+')');
			    console.log(data)
				for(i in data) {
					str = "<tr>" +
                        ...
                        "<td>"+"<a href=\"<?php echo U('Product/homeProduct','','');?>/pid/"+data[i]['pid']+"\" target=\"_blank\">" + data[i].bclassc_number +'.'+ data[i].sclassc_number +'.'+data[i].snumber_number+"</a>"+"</td>" +
						"<td>"+"<a href=\"<?php echo U('Product/homeProduct','','');?>/pid/"+data[i]['pid']+"\" target=\"_blank\">" + data[i].namezh + "</a>"+"</td>" +
							"<td>" + data[i].price + "</td>" +
							"<td>" + data[i].number + "</td>" +
							"<td style='padding:0;'>" + "<input name='number[]' class='form-control' placeholder='<?php echo (L("please_enter")); ?>' type='text' value='" + data[i].number + "'/>" + "</td>" +
	...
	function dogoods(form) {
		...
		var oid = $('#oidm').val();
        if(val==null){
            $('#node_message').html('<?php echo (L("nothing_is_selected")); ?>');
            node_message.style.display = 'block';
            var t = setTimeout("node_message.style.display='none';", 2000);
            return false;
        }
		$.ajax({
			url: "<?php echo U('Warehouse/collectDo');?>",
			type: 'post',
			dataType: "json",
			async: true,
			data: {
			...
			success: function(data) {
                ...
                if(data.info) {
					...
					$('#node_message').html('<?php echo (L("successful_receipt")); ?>');
					node_message.style.display = 'block';
					var t = setTimeout("node_message.style.display='none';location.reload()", 2000);
				}else if(data=='no') {
					$('#node_message').html('<?php echo (L("nothavewarehouse")); ?>');
					node_message.style.display = 'block';
					var t = setTimeout("node_message.style.display='none';", 2000);
				} else {
					$('#node_message').html('<?php echo (L("goods_receipt_failure")); ?>');
					node_message.style.display = 'block';
				}
			}
		});
