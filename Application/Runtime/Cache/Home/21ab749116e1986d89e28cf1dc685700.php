    ...
    <div class="col-md-12">
        <section class="panel">
            <header class="panel-heading">
                ...
                <h2 class="panel-title">收货异常处理</h2>
            </header>
            <div class="panel-body">
                <div id="node_message" style="position: fixed;top: 50%;left: 50%;display: none;background: #888;min-width:300px;margin-left: -150px;margin-top:-15px;border-radius: 8px;text-align: center;font-size: 16px;padding: 12px 20px;color: #fff;z-index: 1200;"></div>
                <form class="" action="<?php echo U('PurchaseOrder/bnormaGoods');?>" method="get" style="display: inline-block;float: left;margin-right:15px ;">
                    <div class="input-group" style="width: 500px;">
						<span class="input-group-addon">
                    		订单
                		</span>

                        <input name="search" type="text" value="<?php echo ($odnumber); ?>" class="form-control" placeholder="请输入订单号">
                        <span class="input-group-btn">
                ...
                <table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;margin-top: 20px;">
                    ...
                    <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
                            <td><?php echo ($k); ?></td>
                            <td><a href="<?php echo U('PurchaseOrder/bnormaGoodsdetails',array('id'=>$vo['order_purchase_success_id']));?>" target="_blank">PO<?php echo ($vo["order_number"]); ?></a></td>
                            <td><a href="<?php echo U('Supplier/detail',array('id'=>$vo['supplier_id']));?>" target="_blank"><?php echo ($vo["namezh"]); ?></a></td>
                            <td><?php echo ($vo["nums"]); ?></td>
                            <td><?php echo ($vo["actual_nums"]); ?></td>
                            <td><?php echo ($vo["creationtime"]); ?></td>
                            <td>
                                <a href="<?php echo U('PurchaseOrder/bnormaGoodsdetails',array('id'=>$vo['order_purchase_success_id']));?>" target="_blank" style="border: 0px;color: #fff;" class="btn btn-primary btn-xs">详情</a>
                            </td>
