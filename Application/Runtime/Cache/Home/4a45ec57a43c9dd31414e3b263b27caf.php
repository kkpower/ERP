    ...
    <div class="col-md-12">
        <section class="panel">
            ...
            <div class="panel-body">
                ...
                <form class="" action="<?php echo U('System/noticelist','','');?>" method="get" style="display: inline-block;float: left;margin-right:15px ;">
                ...
                <table id="table1" class="table table-hover table-bordered table-striped" style="margin-top: 20px;border-collapse: collapse;">
                    ...
                    <?php if(is_array($info)): $i = 0; $__LIST__ = $info;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo ($vo["textzh"]); ?></td>
                            <td><?php echo ($vo["textus"]); ?></td>
                            <td>
                                <div title="<?php echo (L("Modify")); ?>" class="btn btn-primary btn-xs" onclick="edit('<?php echo ($vo["id"]); ?>')"><span class="glyphicon glyphicon-pencil"></span></div>
                            </td>
...
<script>
    ...
    function edit(id) {
        $('#id').val(id);
        $.ajax({
            url: "<?php echo U('System/noticedetails');?>", //通过页面元素的ID来获取设备ID
            type: "post", //选择传值方式
            data: {
                id: id
            },
    ...
    function update(form) {
        ...
        $.ajax({
            url: "<?php echo U('System/noticeedit');?>", //通过页面元素的ID来获取设备ID
            type: "post", //选择传值方式
            data: {
                id: $('#id').val(),
                textzh:$('#textzh').val(),
