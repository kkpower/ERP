	...
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title"><?php echo (L("Keyword_management")); ?></h2>
			</header>
			<div class="panel-body">
				<form id="form1" style="float: left;" action="<?php echo U('Product/keywordsList');?>" method="get">
					<div class="input-group" style="width: 400px;float: left;">
						<span class="input-group-addon">
		                    	<?php echo (L("key_words")); ?>
		                </span>
						<input name="keywords" type="text" value="<?php echo ($search); ?>" class="form-control" placeholder="<?php echo (L("query_keywords")); ?>">
						<span class="input-group-btn">
		                    <button type="submit" class="btn btn-success" style="white-space: nowrap;"><?php echo (L("Search")); ?></button>
		                </span>
					</div>
					<div class="input-group" style="float: left;width: 210px;margin-left: 15px;">
						<span class="input-group-addon">
				...
				<!--<button class="btn btn-primary" style="float: left;margin-left: 15px;" onclick="add_keywords()"><?php echo (L("Add_keywords")); ?></button>-->
				<table id="table1" class="table table-hover table-bordered table-striped" style="text-align: center;border-collapse: collapse;">
					<thead>
						<tr>
							...
							<td style="text-align: center"><?php echo (L("key_words")); ?></td>
							<td style="text-align: center"><?php echo (L("Operation")); ?></td>
						</tr>
					</thead>
					<tbody id="transport">
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								...
								<td>
									<button type="button" title="<?php echo (L("Modify")); ?>" class="btn btn-primary btn-xs" onclick="edit_keywords(this,'<?php echo ($vo["id"]); ?>')">
										<span class="glyphicon glyphicon-pencil"></span>
									</button>
									<button type="button" title="<?php echo (L("Delete")); ?>" class="btn btn-danger btn-xs" onclick="del_keywords(this,'<?php echo ($vo["id"]); ?>')">
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">
					<?php echo (L("Add_keywords")); ?>
                </h4>
			</div>
			<div class="modal-body">
				<div class="input-group" style="padding: 0 20px 0;">
					<span class="input-group-addon"><?php echo (L("key_words")); ?></span>
					<input type="text" id="add_keywords" class="form-control" placeholder="<?php echo (L("enter_new_keywords")); ?>(<?php echo (L("English_is_not_case_sensitive")); ?>)">
				</div>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" onclick="add_save()"><?php echo (L("Save")); ?></button>
	...
	<div class="modal-dialog" style="width: 500px;">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="input-group" style="padding: 0 20px 0;">
					<span class="input-group-addon"><?php echo (L("key_words")); ?></span>
					<input type="text" id="keywords" class="form-control" placeholder="<?php echo (L("Please_enter_a_keyword")); ?>">
				</div>
			</div>
			<div class="modal-footer">
				<button type="button" class="btn btn-primary" onclick="change_keywords()"><?php echo (L("Save")); ?></button>
				<button type="button" class="btn btn-default" data-dismiss="modal"><?php echo (L("Cancel")); ?></button>
			</div>
		</div>
		<!-- /.modal-content -->
...
<script>
	...
	function add_save() {
		if($('#add_keywords').val() == '') {
			...
			$('#node_message').text('<?php echo (L("Keywords_cannot_be_empty")); ?>');
			node_message.style.display = 'block';
			var t = setTimeout("node_message.style.display='none'", 2000);
			return false;
		}
		$.ajax({
			url: "<?php echo U('Product/addKeywords');?>", //通过JQ获取URL获得路径
			data: {
	...
	//删除关键词
	function del_keywords(){
		
	}
	//修改关键词
	function change_keywords() {
		$.ajax({
			url: "<?php echo U('Product/editKeywords');?>", //通过JQ获取URL获得路径
			data: {
				id: $('#keywords_id').val(),
			...
			success: function(data) {
				...
				if(data == 1) {
					...
					$('#node_message').text('<?php echo (L("Successfullymodified")); ?>');
					node_message.style.display = 'block';
					var t = setTimeout("node_message.style.display='none';location.reload();", 2000);
				} else if(data == 0) {
					if(t) {
					...
					$('#node_message').text('<?php echo (L("Keyword_repetition")); ?>，<?php echo (L("fail_to_edit")); ?>');
					node_message.style.display = 'block';
					var t = setTimeout("node_message.style.display='none';", 2000);
				}
			}
