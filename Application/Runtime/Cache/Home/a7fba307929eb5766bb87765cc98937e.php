...
<input type="hidden" id="purchaseOrderdownPdf" value="<?php echo U('PurchaseOrder/purchaseOrderdownPdf','','');?>">
<div class="row">
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title"><?php echo (L("purchase_order_review")); ?></h2>
			</header>
			<div class="panel-body">
				<div id="node_message" style="position: fixed;top: 50%;left: 50%;display: none;background: #888;min-width:300px;margin-left: -150px;margin-top:-15px;border-radius: 8px;text-align: center;font-size: 16px;padding: 12px 20px;color: #fff;z-index: 1200;"></div>
				<!--<form class="" action="<?php echo U('PurchaseOrder/paymentOrder');?>" method="get" style="display: inline-block;float: left;margin-right:15px ;">-->
				...
				<table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
					<thead>
						<tr>
							...
							<th><?php echo (L("firstinstance")); ?></th>
							<th><?php echo (L("initial_review_time")); ?></th>
					...
					<tbody>
						<?php if(is_array($purchaselist)): $k = 0; $__LIST__ = $purchaselist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
								...
								<td>
									<button onclick="toexamine('<?php echo ($vo["id"]); ?>','<?php echo ($vo["warehouse_id"]); ?>','<?php echo ($vo["supplier_id"]); ?>','<?php echo ($vo["order_number"]); ?>','<?php echo ($vo["transport"]); ?>')" style="border: 0px;" type="button" data-toggle="modal" class="btn btn-primary btn-xs" " >
									<?php echo (L("Review")); ?>
                                </button>
                                <a href="<?php echo U( 'PurchaseOrder/productDetailstwo',array( 'id'=>$vo['id']));?>" target="_blank" style="border: 0px;color: #fff;" class="btn btn-primary btn-xs"><?php echo (L("Details")); ?></a>
								</td>
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<!--<h4 class="modal-title">-->
                        <!--采购订单号:<span style="color: red;">PO</span><span id="order_number1" style="color: red;"></span>-->
                    <!--</h4>-->
				</div>
				...
				<div class="modal-footer">
					...
					<button class="btn btn-primary" id="add_ok" type="submit"><?php echo (L("agree")); ?></button>
					<button type="submit" class="btn btn-default" id="add_no"><?php echo (L("refuse")); ?></button>
				</div>
			</div>
...
<script>
	...
	function toexamine(id, warehouse_id, supplier_id, order_number, transport) {
		...
		$.ajax({
			url: "<?php echo U('PurchaseOrder/reviewOrders');?>",
			type: "post",
			data: {
				id: id
			},
			...
			success: function(data) {
				...
				order += "<tr>" +
					...
					"<td>" + data['info']['list'][0]['lastname'] + data['info']['list'][0]['namezhs'] + "</td>" +
					"<td>" + data['info']['list'][0]['examine_time'] + "</td>" +
					"</tr>";
				...
				for(i in data['list']['list']) {
					str += "<tr>" +
						"<input name='pid[]' type='hidden' value='" + data['list']['list'][i].supplier_pr_id + "'/>" +
						"<input name='number[]' type='hidden' value='" + data['list']['list'][i].num + "'/>" +
						"<input name='price[]' type='hidden' value='" + data['list']['list'][i].price + "'/>" +
						"<input name='total[]' type='hidden' value='" + data['list']['list'][i].total + "'/>" +
						"<input name='currency[]' type='hidden' value='" + data['list']['list'][i].currency + "'/>" +
	...
	function doexamine(form) {
		$.ajax({
			url: "<?php echo U('PurchaseOrder/examineTo');?>",
			type: "post",
			data: $(form).serialize(),
			dataType: "json",
			async: true,
			success: function(data) {
				if(data.status == 0) {
					...
					$('#node_message').html('<?php echo (L("audit_failure")); ?>');
					node_message.style.display = 'block';
					var t = setTimeout("node_message.style.display='none';", 2000);
				} else if(data == 'SO') {
					$('#node_message').html('<?php echo (L("successful_review")); ?>');
