	...
	<div class="col-md-12">
		<section class="panel">
			<header class="panel-heading">
				...
				<h2 class="panel-title"><?php echo (L("Receivingwarehouse")); ?>&nbsp;--&nbsp;<?php if(($_COOKIE['think_language']) == "zh-cn"): echo ($list[0]['zkname']); endif; ?>
					<?php if(($_COOKIE['think_language']) == "en-us"): echo ($list[0]['englishname']); endif; ?></h2>
			</header>


			<div class="panel-body">
				<div id="node_message" style="position: fixed;top: 50%;left: 50%;display: none;background: #888;min-width:300px;margin-left: -150px;margin-top:-15px;border-radius: 8px;text-align: center;font-size: 16px;padding: 12px 20px;color: #fff;z-index: 1200;"></div>
				<form class="" action="<?php echo U('Warehouse/detailsTransportation');?>" method="get" style="display: inline-block;float: left;margin-right:15px ;">
					<input type="hidden" name="pid" value="<?php echo ($pid); ?>">
					<input type="hidden" name="kid" value="<?php echo ($kid); ?>">
					<div class="input-group" style="width: 500px;">
						<input name="o_id" type="text" value="<?php echo ($oid); ?>" class="form-control" placeholder="<?php echo (L("Pleaseordernumber")); ?>">
						<span class="input-group-btn">
                    <button type="submit" class="btn btn-success" style="white-space: nowrap;"><?php echo (L("Search")); ?></button>
                </span>
					</div>
				</form>
				<!--<input type="hidden" value="<?php echo ($mold); ?>" id="mold">-->
				...
				<!--<form onsubmit="return onm(this)">-->
					<!--<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>-->
						<!--<input type="hidden" value="<?php echo ($vo["number"]); ?>" name="number[]">-->
						<!--<input type="hidden" value="<?php echo ($vo["o_id"]); ?>" name="oid[]">-->
						<!--<input type="hidden" value="<?php echo ($vo["g_id"]); ?>" name="gid[]">-->
					<!--<?php endforeach; endif; else: echo "" ;endif; ?>-->
					<!--<input type="hidden" value="<?php echo ($pid); ?>" name="pid">-->
					<!--<input type="hidden" value="<?php echo ($mold); ?>" name="mold">-->
					<!--<input type="hidden" value="<?php echo ($kid); ?>" name="warehouse_id">-->
				<!--</form>-->
				<table id="table1" class="table table-hover mb-none table-striped table-bordered" style="border-collapse: collapse;">
					<thead>
					<h5 style="font-weight: bold;"><span style="margin-right: 10px;color: #DC143C;"><?php echo (L("product_number")); ?>:</span><a href="<?php echo U('Product/homeProduct',array('pid'=>$list[0]['pid']));?>" target="_blank"><?php echo ($list[0]["bclassc_number"]); ?>.<?php echo ($list[0]["sclassc_number"]); ?>.<?php echo ($list[0]["snumber_number"]); ?></a></h5>
						...
						<h5 style="font-weight: bold;"><span style="margin-right: 10px;color: #DC143C;"><?php echo (L("unitpriceofproduct")); ?>:</span><?php echo ($list[0]["price"]); ?>&nbsp;&nbsp;RMB</h5>
						<tr>
					...
					<tbody>
						<?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><tr>
								...
								<td>
									<?php if($vo["order_type"] == 采购订单 ): ?><a href="<?php echo U('PurchaseOrder/completedDetails',array('id'=>$vo['o_id']));?>" target="_blank">PO<?php echo ($vo["order_number"]); ?></a><?php endif; ?>
								</td>
